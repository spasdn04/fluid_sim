...
CYLINDER_CENTER_INDEX_X2 = N_POINTS_X // 2
CYLINDER_CENTER_INDEX_Y2 = N_POINTS_Y // 2

CYLINDER_RADIUS_INDICES = N_POINTS_Y // 9

MAX_HORIZONTAL_INFLOW_VELOCITY = 0.1

VISUALIZE = True
PLOT_EVERY_N_STEPS = 100
SKIP_FIRST_N_ITERATIONS = 1000
...
def main():
    ...
    obstacle_mask2 = (
        jnp.sqrt(
            (
                X
                -
                CYLINDER_CENTER_INDEX_X2
            )**2
            +
            (
                Y
                -
                CYLINDER_CENTER_INDEX_Y2
            )**2
        )
        <
            CYLINDER_RADIUS_INDICES
    ...
    def update(discrete_velocities_prev):
        ...
        for i in range(N_DISCRETE_VELOCITIES):
            discrete_velocities_post_collision =\
                discrete_velocities_post_collision.at[obstacle_mask1, LATTICE_INDICES[i]].set(
                    discrete_velocities_prev[obstacle_mask1, OPPOSITE_LATTICE_INDICES[i]]
                )
